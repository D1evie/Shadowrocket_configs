...
const us  = proxies.filter(p => /@US\b/i.test(p.tag)).map(p => p.tag);
const nl  = proxies.filter(p => /@NL\b/i.test(p.tag)).map(p => p.tag);
const pl  = proxies.filter(p => /@PL\b/i.test(p.tag)).map(p => p.tag);
const de  = proxies.filter(p => /@DE\b/i.test(p.tag)).map(p => p.tag);
